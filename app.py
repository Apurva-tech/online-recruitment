...
# Adding home.html route to '/'
@app.route('/')
def home():
	return render_template('home.html', app_data=app_data)

# Logic for the signup page using POST methods
...
# Logic to redirect user to account after loging in
# The account template is rendered according the configured data from db
...
# Logic to display username on using '/user/<username>' route
...
# Logic to display usertype on using '/configure/<user_type>' route
...
# Logic to create a new post by checking if user type is Company
...
# Apply on a job post by sending a application form 
...
# Prompt message to confirm if the application is submitted		
...
# Scheduling a interview for a job seeker
...
# Logic to send offer letter to job seeker if hired
...
# Finding jobs rendering findjob.html 
...
# Redirect user to home page after loging out 
@app.route('/logout')
def logout():
	if 'user' in session:
...
# Mark debug = DEVELOPMENT_ENV for enabling debug while running the server
